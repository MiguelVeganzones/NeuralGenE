...
namespace ga_snn
{
  ...
  */
  ...
  template <typename T, size_t Batch_Size, Layer_Structure Structure>
    requires(std::is_floating_point_v<T>and Batch_Size > 0)
  class layer
  {
  private:
  ...
  public:
    using layer_weights_shape = ga_sm::static_matrix<T, s_Inputs, s_Outputs>;
  ...
  public:
    ...
    {
      // return multiply_add_activate(m_layer_weights_mat, Input, m_offset_vector,
      // s_Activation_Function);

      return s_Activation_Function(matrix_vec_add(matrix_mul(Input, m_layer_weights_mat), m_bias_vector));
    }
    ...
    {
      m_layer_weights_mat.mutate_replace_add(params.p1, params.p2, params.Avg, params.Stddev, fn, args...);
    ...
    [[nodiscard]] inline constexpr const layer_weights_shape& get_layer_weights_mat() const
    {
      return m_layer_weights_mat;
    }

    [[nodiscard]] inline constexpr layer_weights_shape& get_layer_weights_mat()
    {
      return m_layer_weights_mat;
    }
  ...
  {
    os << layer.get_layer_weights_mat() << layer.get_bias_vector() << "\n";
    return os;
  }
  ...
  template <typename T, size_t Batch_Size, Layer_Signature... Signatures>
    requires((sizeof...(Signatures) >= 3) and std::is_floating_point_v<T>and Batch_Size > 0)
  ...
  {
    in_place_x_crossover(layer1.get_layer_weights_mat(), layer2.get_layer_weights_mat());
  ...
  {
    double const distance = normalized_L1_distance(layer1.get_layer_weights_mat(), layer2.get_layer_weights_mat()) +
