...
public:
    ...
    using activation_function = matrix_activation_functions::
        activation_function<output_vector_shape, Structure.Activation>;
...
public:
    ...
    [[nodiscard]]
    constexpr output_vector_shape forward_pass(input_vector_shape const& Input
    ) const
    {
        auto out =
            matrix_vec_add(matrix_mul(Input, m_weights_mat), m_bias_vector);
...
template <typename T>
concept static_layer_type =
    requires { layer_dummy(std::declval<std::remove_cvref_t<T>&>()); };
...
struct layer_unroll<T, Inputs, Batch_Size, Current_Signature>
{
    ...
    using current_layer_type = layer<
        T,
        Batch_Size,
        Layer_Structure{ s_Inputs, s_Outputs, Current_Signature.Activation }>;
    ...
    constexpr auto get() -> current_layer_type&
    {
        ...
        {
            static_assert(
                false && Idx, "requested layer index exceeds number of layers"
            );
        }
    }
    ...
    {
        layer_idx == 0 ? m_Data.mutate(std::forward<Fn>(fn))
                       : std::unreachable();
    }
    ...
    [[nodiscard]]
    auto forward_pass(
        ga_sm::static_matrix<T, Batch_Size, s_Inputs> const& input_data
    ) const
    {
    ...
    static std::size_t layer_size(const int idx_to_target_layer) noexcept
    {
        ...
        assert_unreachable();
        // return idx_to_target_layer == 0 ? current_layer_type::layer_size() :
        // std::unreachable();
    }
    ...
    static std::size_t parameter_count(const int idx_to_target_layer) noexcept
    {
        ...
        assert_unreachable();
        // return idx_to_target_layer == 0 ?
        // current_layer_type::parameter_count() : std::unreachable();
    }
};
...
struct layer_unroll<T, Inputs, Batch_Size, Current_Signature, Signatures...>
{
    ...
    constexpr auto& get() noexcept
    {
        ...
        {
            return m_Next.template get<Idx - 1>(
            ); // if the index is not 0, ask the
    ...
    {
        layer_idx == 0 ? m_Data.mutate(std::forward<Fn>(fn))
                       : m_Next.mutate_layer(--layer_idx, std::forward<Fn>(fn));
    }
...
private:
    using layers_type =
        layer_unroll<T, s_Input_Size, Batch_Size, Signatures...>;
...
public:
    ...
    [[nodiscard]]
    auto layer() -> decltype(m_Layers.template get<Idx>()
    )& // TODO: maybe try decltype(auto)
    {
    ...
    template <typename Fn>
    auto mutate_set_layers(const std::vector<size_t>& layers_idx, Fn&& fn)
        -> void
    {
    ...
    auto mutate_set_layers_impl(const std::vector<size_t>& layers_idx, Fn&& fn)
        -> void
    {
        ...
        requires(std::is_trivial_v<static_neural_net> && std::is_standard_layout_v<static_neural_net>)
    {
        ...
        {
            out.write(
                reinterpret_cast<const char*>(&layer_signature.Size),
                sizeof(layer_signature.Size)
            );
        }
    ...
    auto load(const std::filesystem::path& filename) -> void
    {
        ...
        for (const auto& layer_signature : s_Signatures)
        {
            ...
            {
                const auto message =
                    "Cannot load this net here. Shapes must match.\n";
                std::cout << message;
                log::add(message);
                std::exit(EXIT_FAILURE);
            }
        ...
        requires(std::is_trivial_v<static_neural_net> && std::is_standard_layout_v<static_neural_net>)
    {
        ...
        for (auto* p = reinterpret_cast<char*>(this);
    ...
    {
        return m_Layers.forward_pass(
            cast_to_shape<Batch_Size, s_Input_Size>(input_data)
        );
    }
    ...
    [[nodiscard]]
    auto forward_pass(ga_sm::static_matrix<T, M_In, N_In> const& input_data
    ) const -> ga_sm::static_matrix<T, M_Out, N_Out>
    {
    ...
    {
        return m_Layers.forward_pass(ga_sm::static_matrix<value_type, 1, 1>{
            input_value })(0, 0);
    }
...
template <typename T>
concept static_neural_net_type =
    requires(T&& t) { nnet_dummy(std::declval<std::remove_cvref_t<T>&>()); };

template <typename T, typename... Ts>
concept same_structure_net_concept =
    requires(T t, Ts... ts) { (t.s_Signatures == ... == ts.s_Signatures); };
...
) -> void
{
    ...
    if constexpr (I != NNet::s_Layers - 1)
        to_target_net_x_crossover<NNet, I + 1>(
            in_net1, in_net2, out_net1, out_net2
        );
}

/**
 * \brief Divides NNet layers in two groups (can be thought as encoder and
 * decoder) and crossovers those layer groups in out_net1 and out_net2 \note
 * Beware of bad code
 */
template <static_neural_net_type NNet>
...
) -> void
{
    ...
    assert(
        net_parameters * sizeof(value_type) ==
        first_half_size + second_half_size
    );
    assert(net_parameters * sizeof(value_type) == sizeof(NNet));

    std::memcpy(&out_net1, &in_net1, first_half_size);
    std::memcpy(
        (value_type*)&out_net1 + first_half_count,
        (value_type*)&in_net2 + first_half_count,
        second_half_size
    );

    std::memcpy(&out_net2, &in_net2, first_half_size);
    std::memcpy(
        (value_type*)&out_net2 + first_half_count,
        (value_type*)&in_net1 + first_half_count,
        second_half_size
    );
}
...
/**
 * \brief Divides NNet layers in two groups (can be thought as encoder and
 * decoder) and crossovers those layer in place \note Beware of bad code
 */
template <static_neural_net_type NNet>
    requires(std::is_standard_layout_v<std::remove_reference<NNet>> && std::is_trivial_v<std::remove_reference<NNet>>)
...
) -> void
{
    ...
    const auto end =
        first ? first_half_size : first_half_size + second_half_size;
    auto p1 = (char*)&net1;
    auto p2 = (char*)&net2;
...
) -> ga_sm::static_matrix<R, N, N>
{
    ...
    for (size_t j = 0; j != N - 1; ++j)
    {
        ...
        {
            const auto distance = neural_net_distance<R>(
                net_ptr_arr[j].get(),
                net_ptr_arr[i].get(),
...
/**
 * \brief Returns the sum of the normalized distance between homologous pairs of
 * matrices between two layers \tparam R \tparam Layer \param layer1 \param
 * layer2 \return
 */
template <
    std::floating_point R,
    static_layer_type   Layer1,
    static_layer_type   Layer2,
    typename Distance>
    requires(
        (Layer1::s_Inputs == Layer2::s_Inputs) &&
        (Layer1::s_Outputs == Layer2::s_Outputs) &&
...
) -> R
{
    return matrix_distance<R>(
               layer1.get_weights_mat(),
               layer2.get_weights_mat(),
               std::forward<Distance>(dist_op)
           ) +
        matrix_distance<R>(
               layer1.get_bias_vector(),
               layer2.get_bias_vector(),
               std::forward<Distance>(dist_op)
        ) +
        activation_function_distance<R>(
               layer1.get_activation_function(),
...
/**
 * \brief Calculates the sum of the normalized distance between homologous pairs
 * of matrices between both nets. Distance between each matrix is normalized by
 * the amount of elements in the matrices. \tparam R Float return trype \tparam
 * NNet1 Net type 1. Must share all layer shapes with Net type 2 as well as the
 * amount of layers. \tparam NNet2 Net type 2. Must share all layer shapes with
 * Net type 1 as well as the amount of layers. \tparam I Implementation detail,
 * should not be used. Used to index layers recursively. \param net1 First net
 * to compare \param net2 Second net to compare \return Sum of the normalized
 * distance between pairs of matrices between both nets.
 */
template <
    std::floating_point    R,
...
) -> R
{
    const auto current_distance = layer_distance<R>(
        net1.template layer<I>(),
        net2.template layer<I>(),
        std::forward<Distance>(dist_op)
    );
    ...
    {
        return current_distance +
            neural_net_distance<R, NNet1, NNet2, Distance, I + 1>(
                   net1, net2, std::forward<Distance>(dist_op)
            );
    }
}
