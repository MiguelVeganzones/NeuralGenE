...
{
public:
    static inline void add(const std::string sv)
    {
        s_log += sv + '\n';
        ++s_logs;
    }
...
private:
    static void init()
    {
        if (const auto error_code = ::localtime_r(&s_Compilation_epoch, &s_tm); error_code == 0)
        {
    ...
    static inline const std::time_t s_Compilation_epoch = std::time(nullptr); // Current time
